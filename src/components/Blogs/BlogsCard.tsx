...
const BlogDate: React.FC<{ date: string }> = ({ date }) => {
  return (
    <p className="text-foreground shadow-custom-inset-shadow-dark mb-2 w-fit rounded p-1 text-xs">
...
const Title: React.FC<React.PropsWithChildren<{}>> = ({ children }) => {
  return (
    <h2 className="text-foreground mb-2 w-full max-w-xl text-base font-bold tracking-tight md:max-w-full">
      {children}
    </h2>
  );
};
